...
state = {"busy": False, "last": "", "torque": True}; lock = threading.Lock(); io = threading.Lock()   # io: one bus transaction at a time
robot = connect(); print("arm connected", flush=True)
rest = np.array(json.load(open(os.path.join(HERE, "motions_tuned.json")))["REST"]["q"][0])

def send(q):
    with io: robot.send_action({f"{j}.pos": float(q[k]) for k, j in enumerate(JOINTS)})
def pose():
    with io: return read_pose(robot)
def torque(on):
    with io: (robot.bus.enable_torque() if on else robot.bus.disable_torque())
def ease_to(target, max_speed=EASE_SPEED, min_t=0.15):
    cur = np.array(pose()); tgt = np.array(target, float)
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        self._json({"busy": state["busy"], "last": state["last"], "torque": state["torque"], "pose": [round(x, 1) for x in pose()]})
    def do_POST(self):
        ...
        try:
            ...
            elif self.path == "/release": torque(False); state["torque"] = False; state["last"] = "torque off: move the arm by hand"
