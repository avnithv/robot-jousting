...
// the whole track loops through the music bus (so the "music" toggle still mutes it), moods and intensity only shape its
...
export const music = {
  ...
  start(m = 'town') {
    ...
    running = true; apply();
    if (el.paused) el.play().catch(() => {});   // the whole track, from the top, looping (the static server has no range support, so no seeking)
  },
