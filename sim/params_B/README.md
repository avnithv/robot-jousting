...
HARD RULE: shoulder_lift never below -89 deg (measured on the real arm: leaning further back rests the upper arm on the board). tune.py nudges violating key poses to the nearest safe pose and warns if a trajectory dips between keys.

This folder is ARM B's (SO-101) own parameter set. Nothing is shared with arm A (sim/params/). Trajectories generate as MOVE@B.
